...
from pincrawl.scrapers.scraper import ScrapeResult, LinksResult, UnrecoverableScrapingError


class TestProxyScraper:

    @patch.dict(os.environ, {'PROXY_USERNAME': 'testuser', 'PROXY_PASSWORD': 'testpass'})
    def test_init_success(self):
        """Test successful initialization with valid credentials."""
        scraper = ProxyScraper(timeout=30)
        assert scraper._proxy == 'http://testuser:testpass@gate.decodo.com:7000'

    @patch.dict(os.environ, {}, clear=True)
    def test_init_missing_credentials(self):
        """Test initialization fails without credentials."""
        with pytest.raises(ValueError, match="PROXY_USERNAME and PROXY_PASSWORD"):
            ProxyScraper()

    @patch.dict(os.environ, {'PROXY_USERNAME': 'testuser'}, clear=True)
    ...
    def test_scrape_success(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            'https://example.com',
            proxies={'http': 'http://testuser:testpass@gate.decodo.com:7000',
                    'https': 'http://testuser:testpass@gate.decodo.com:7000'}
        )
    ...
    def test_scrape_with_xpath(self, mock_selector_class, mock_bs, mock_get):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.text = '<html><body><div class="content">Extracted</div></body></html>'
        mock_response.content = b'<html><body><div class="content">Extracted</div></body></html>'
        mock_get.return_value = mock_response
        ...
        scraper = ProxyScraper()
        result = scraper.scrape('https://example.com', xpath='//div[@class="content"]')
    ...
    def test_scrape_xpath_no_match(self, mock_selector_class, mock_get):
        ...
        scraper = ProxyScraper()
        result = scraper.scrape('https://example.com', xpath='//div[@class="notfound"]')
